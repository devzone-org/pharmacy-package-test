...
class StockNearExpiry extends Component
{
    ...
    {
        $products = new LengthAwarePaginator([], 0, 100);
        ...
        if ($this->searching) {
            ...
            // --- compute last sold and expiring_in fields ---
            $pi_ids = $products->pluck('pi_id')->toArray();
            $last_solds = SaleDetail::from('sale_details as sd')
            ...
            foreach ($products as $product) {
                $product->last_sold = $last_solds[$product->pi_id] ?? null;
