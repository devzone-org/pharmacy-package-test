...
class OpenReturns extends Component
{
    ...
    {
        return view('pharmacy::livewire.sales.open-returns');
    }
    ...
    {
        $this->validate();
        $this->total_after_deduction = collect($this->returns)->sum('total_cost') - (collect($this->returns)->sum('total_cost')*$this->deduction/100);
        $this->total_after_deduction = number_format($this->total_after_deduction, 2);
        ...
        try {
            ...
            foreach ($this->returns as $k => $pro){
                ...
                $des = "Open Return of PKR " . number_format($pro['total_cost'],2) . "/- on date " . date('d M, Y H:i:s') .
                    " with " . $this->deduction . "% deduction on total of PKR " . number_format($this->total_after_deduction,2) . "/-.";
