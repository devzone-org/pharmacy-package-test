    ...
    public function mount($purchase_id)
    {
        ...
        foreach ($details as $data) {
            $this->order_list[] = [
                ...
                'bonus' => 0,
                'disc' => 0,
                'cost_of_price' => round($data['cost_of_price'] * $data['packing']),
                'after_disc_cost' => round($data['cost_of_price'] * $data['packing']),
                'retail_price' => round($data['retail_price'] * $data['packing']),
                'total_retail_price' => round($data['retail_price'] * ($data['packing']) * ($data['qty'] / $data['packing'])) ,
                'profit' =>round($data['retail_price'] * ($data['packing']) * ($data['qty'] / $data['packing']) - $data['cost_of_price'] * ($data['packing']) * ($data['qty'] / $data['packing']) ) ,
                'salt' => $data['salt'],
                'total_cost' => round($data['cost_of_price'] * ($data['packing']) * ($data['qty'] / $data['packing'])),
